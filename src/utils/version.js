...
export const getVersionInfo = async(includePreRelease = false) => {
  ...
  const assets = release.assets.filter(asset => /-v.*-(?:universal|arm64-v8a|armeabi-v7a|x86_64|x86)\.apk$/.test(asset.name))
...
export const downloadNewVersion = async(assets, onDownload = noop) => {
  ...
  let savePath = temporaryDirectoryPath + '/lx-music-car.apk'
...
export const updateApp = async() => {
  ...
  await installApk(apkSavePath, `${await getPackageName()}.provider`)
}
