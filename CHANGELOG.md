...
- 修复 React Native 的 `StatusBar` 组件在车机页面挂载后可能覆盖导航层设置、重新显示状态栏的问题。
- 车机版不再渲染该组件；首页、播放详情、歌单详情和其他复用页面均不会再留下无内容的顶部纯色占位，右侧原车悬浮窗可使用完整顶部空间。

## [1.8.25] - 2026-08-01

### 浏览优先的横屏信息栏

- 缩小主页全局信息栏：52dp 高度、20sp 页面标题，以及 84dp 的紧凑主题切换，避免标题和主题开关占据歌单浏览空间。
- 搜索、在线歌单和“我的列表”的二级固定栏统一收敛至 56dp；歌单排序、分类、打开歌单等子控件同步跟随，释放约 48dp 垂直空间给实际内容。

## [1.8.24] - 2026-08-01

### 无系统栏驾驶界面

- 按实车视觉反馈，车机再次隐藏状态栏与导航栏，移除顶部白条及其对右上原车系统卡片造成的不协调视觉。
- 原生 Activity 不恢复强制沉浸式或全屏标志；仅由导航层统一管理各页面和弹层的系统栏可见性，保留 Launcher 对右侧区域的控制权。

## [1.8.23] - 2026-08-01

### 原车状态栏与导航栏分离

- 修正 v1.8.21 的窗口策略：车机保留系统状态栏提供顶部窗口边界，但继续隐藏 Android 导航栏，避免原车 Launcher 的左侧 Dock 被应用主动唤出。
- 该策略同时覆盖首页、播放详情、歌单详情、评论页和应用内弹层，避免切换页面后窗口策略不一致。

## [1.8.22] - 2026-07-31

### 驾驶侧栏信息层级

- 左上原车主页入口已经承担“返回主页”职责，车机侧栏不再重复显示应用内主页按钮。
- 设置和退出应用收进底部工具区：缩小图标、字号和行高，保留可达性但不与搜索、歌单、我的列表争夺视觉注意力与可用高度。

## [1.8.21] - 2026-07-31

### 原车窗口协同与侧栏修复

- 修复车机侧栏在原车可用高度较小时裁掉“我的列表”的问题：车机主导航固定保留搜索、歌单、我的列表，设置和退出仍位于底部；榜单不再占用驾驶主导航。
- 车机的首页、播放详情、歌单详情、评论页以及协议、版本、同步模式弹层不再通过导航层隐藏系统状态栏和导航栏；车机不再绘制到系统栏后方，避免任一页面重新触发 Launcher 对右侧系统窗口的压缩。
- 保留横屏、深浅驾驶主题、实体媒体按键及现有车机本地设置边界。

## [1.8.20] - 2026-07-19

### 全页横屏审查与原车系统协同

- 审查搜索、歌单、榜单、设置和歌单详情的横屏容器：统一收紧车机歌单/标签抽屉、榜单侧栏与设置侧栏，确保在原车保留右侧系统区时主内容仍有可用宽度。
- 取消 Activity 获取焦点后强制沉浸式，以及车机 Manifest 中请求隐藏系统栏的 Launcher 元数据；让 VV5 原车系统自行管理右侧悬浮窗和系统栏，避免应用重新获得焦点时压缩该区域。
- 保留横屏、屏幕常亮、可调整任务尺寸与现有媒体按键链路；播放详情页未改动。

## [1.8.19] - 2026-07-19

### 横屏歌单浏览

- 移除歌单页顶部与底部播放栏重复的驾驶概览模块，把完整高度还给当前歌单与歌曲列表；播放详情页保持不变。
- 横屏主内容区统一使用车机页面底色，侧栏压缩至 150dp，底部播放栏调整为 108dp，封面和播放控制同步收敛，避免浏览页被大块固定区域挤压。
- 浅色驾驶主题改为抗强光的灰青表面和更深的边框、文字层级；车机播放控制统一使用车机强调色，提高日间可读性。

## [1.8.18] - 2026-07-18

### 驾驶首页与协同

- 驾驶首页将高频操作收敛为上一首、播放/暂停、下一首三个 72dp 直接动作；不改变已验证可用的实体媒体键链路。
- 歌单“更多”菜单新增“固定到驾驶首页”，最多固定 4 个常用歌单；固定数据仅存于车机，不参与跨端设置同步。
- 明确启动恢复、列表位置恢复、非阻塞同步状态与服务端快照保护边界；手机端会持续刷新在线车机状态。

## [1.8.17] - 2026-07-18

### 车机交互反馈

- 统一歌单、音源、同步和设置所复用的对话框、更多菜单、底部弹层：标题与关闭区域提升至 56dp，菜单项提升至 64dp，默认菜单宽度提升至 180dp。
- 弹层接入深浅驾驶主题的表面、边框、强调色及文字层级，避免业务页已经适配但操作反馈仍保留手机式小控件。
- 未修改已在实车验证可用的 Android 媒体键播放控制链路。

## [1.8.16] - 2026-07-18

### 驾驶首页

- “我的音乐”新增固定驾驶首页卡片：显示当前播放、歌手信息与同步服务连接状态。
- 新增 72dp 的播放/暂停、搜索、歌单快捷动作；无当前歌曲时，播放动作会直接打开歌单选择。
- 保留并明确记录已验证可用的原车实体媒体键控制路径，未改动 Android 音频会话的播放控制。

## [1.8.15] - 2026-07-18

### 车机跨页统一设计

- 在线歌单的来源、分类、排序和“打开歌单”操作统一为 72dp 筛选栏与大字号标签。
- 榜单左侧选择器扩大为 176–240dp，榜单项提升至 64dp，并接入统一深浅主题、活动状态和强调色。
- 设置页的二级导航改为 64dp，左右栏加入车机专属表面与选中态；内容区增加固定留白。
- 设置中的复选框、按钮、输入框和滑块提高到 52–56dp 触控尺度，统一字号与间距。

## [1.8.14] - 2026-07-18

### 车机主页与操作

- 新安装默认从“我的音乐”进入；保留既有用户最后一次主页面选择，避免升级后强制打断使用习惯。
- 侧栏改为主功能与底部设置分组，并缩窄为 164dp，为左侧主应用区释放歌曲列表空间。
- 歌曲行增至 72dp，歌单切换增至 60–64dp，搜索栏增至 72dp，播放栏增至 124dp；播放控制的独立触控区域增至 76dp。
- 车机歌曲、在线搜索结果和歌单列表使用更大的标题/副标题字号、分隔线、当前播放强调色及独立的更多操作按钮。
- 当前播放封面与标题放大，使底部播放栏成为稳定的播放状态锚点。

## [1.8.13] - 2026-07-18

### 车机版

- 新增独立的浅色驾驶主题，与深色夜间主题共用高对比层级、触控尺寸及青绿色状态语义。
- 主页顶栏新增一键“深色 / 浅色”切换；主题选择仅保存在车机本地，不参与跨端同步。
- 主页侧栏、顶栏、搜索区、播放栏和选中态改为使用统一车机调色板；移除对手机背景图和主题色的视觉依赖。
- 更新车机窗口说明：部分 Android 8.1 原车系统会保留右侧原车区域，车机版以左侧主应用区的可读性与可操作性为设计目标。

## [1.8.5] - 2026-07-18

### 车机版

- 将车机版迁移至独立仓库 `Tingfe/lx-music-car`，独立构建、更新和发布。
- 保留 `cn.toside.music.mobile.car` 包名，以支持既有车机版覆盖升级。
- 固定横屏沉浸式全屏，并为车机主页保持屏幕常亮。
- 新增车机驾驶舱布局：带文字的大侧栏、72dp 顶栏、90dp 播放栏及 68dp 播放控制。
- 应用内升级只下载本仓库的车机 APK，并使用当前包名的 FileProvider 安装更新。
